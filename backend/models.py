...
class VideoProcessingTask(BaseModel):
    ...
    detection_results: List[VeinDetectionResult] = Field(default_factory=list, description="检测结果列表")
...
class ProcessingProgressResponse(BaseModel):
    ...
    estimated_time: Optional[float] = Field(default=None, description="预估剩余时间(秒)")
